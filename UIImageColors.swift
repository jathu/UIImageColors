...
extension UIImage {
    ...
    public func getColors(scaleDownSize: CGSize, completionHandler: (UIImageColors) -> Void) {
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0)) {
            ...
            let sortedColorComparator: NSComparator = {
                let m = $0 as? PCCountedColor, o = $1 as? PCCountedColor
                if m?.count < o?.count {
            ...
            CGContextDrawImage(ctx, CGRectMake(0.0, 0.0, CGFloat(width), CGFloat(height)), cgImage)
            ...
            let leftEdgeColorsEnumerator = leftEdgeColors.objectEnumerator()
            var sortedColors = NSMutableArray(capacity: leftEdgeColors.count)
            while let color = leftEdgeColorsEnumerator.nextObject() as? UIColor {
                let colorCount = leftEdgeColors.countForObject(color)
                if randomColorsThreshold < colorCount  {
                    sortedColors.addObject(PCCountedColor(color: color, count: colorCount))
                }
            }
            ...
            if sortedColors.count > 0 {
                proposedEdgeColor = sortedColors[0] as! PCCountedColor
            } else {
            ...
            if proposedEdgeColor.color.isBlackOrWhite && sortedColors.count > 0 {
                for i in 1..<sortedColors.count {
                    let nextProposedEdgeColor = sortedColors[i] as! PCCountedColor
            ...
            let imageColorsEnumerator = imageColors.objectEnumerator()
            sortedColors.removeAllObjects()
            sortedColors = NSMutableArray(capacity: imageColors.count)
            let findDarkTextColor = !result.backgroundColor.isDarkColor
            
            while var color = imageColorsEnumerator.nextObject() as? UIColor {
                color = color.colorWithMinimumSaturation(0.15)
            ...
            for sortedColor in sortedColors {
                let color = (sortedColor as! PCCountedColor).color
